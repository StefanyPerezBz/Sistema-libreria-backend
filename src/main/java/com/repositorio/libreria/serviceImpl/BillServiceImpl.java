        ...
        try {
            ...
            if (validateRequestMap(requestMap)){
                if (requestMap.containsKey("isGenerate") && !(Boolean) requestMap.get("isGenerate")) {
                    ...
                    fileName = LibreriaUtils.getUUID();
                ...
                PdfWriter.getInstance(document, new FileOutputStream(LibreriaConstants.STORE_LOCATION + "\\" + fileName + ".pdf"));
                document.open();
                ...
                JsonArray jsonArray = LibreriaUtils.getJsonArrayFromString((String) requestMap.get("bookDetails"));


                for (int i = 0; i < jsonArray.size(); i++) {
                    addRows(table, LibreriaUtils.getMapFromJson(jsonArray.get(i).getAsJsonObject().toString()));
                }
                document.add(table);
            ...
            return LibreriaUtils.getResponseEntity("Datos requeridos no encontrados", HttpStatus.BAD_REQUEST);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    public ResponseEntity<byte[]> getPdf(Map<String, Object> requestMap) {
        ...
        try {
           ...
           if (!requestMap.containsKey("uuid") && validateRequestMap(requestMap))
               ...
               String filePath = LibreriaConstants.STORE_LOCATION + "\\" + (String) requestMap.get("uuid") + ".pdf";
               if (LibreriaUtils.isFileExist(filePath)) {
    ...
    public ResponseEntity<String> deleteBill(Integer id) {
        try {
            ...
            if (!optional.isEmpty()){
              ...
              return LibreriaUtils.getResponseEntity("La factura ha sido eliminada exitosamente", HttpStatus.OK);
            }
            return LibreriaUtils.getResponseEntity("El ID de la factura no existe", HttpStatus.OK);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        return LibreriaUtils.getResponseEntity(LibreriaConstants.SOMETHING_WENT_WRONG, HttpStatus.INTERNAL_SERVER_ERROR);
